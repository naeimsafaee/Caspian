$(".order-list .bg-line").each(function () {
    $(this).animate(
        {
...
$("#title-tab .nav-tabs a").click(function () {
    var position = $(this).parent().position();
    var width = $(this).parent().width();
    $(" .slider1").css({"left": +position.left, "width": width});
});
var actWidth = $("#title-tab  .nav-tabs").find(".active").parent("li").width();
var actPosition = $("#title-tab  .nav-tabs .active").position();
$("#title-tab  .slider1").css({"left": +actPosition.left, "width": actWidth});
...
$(".order-list-tab a").click(function () {
    $(".order-list-tab a").removeClass("active")
...
$(function () {
    ...
    let sellSliderTradeRange = document.getElementById("sellSliderTradeRange");
    $(".sell .slider").noUiSlider({
        start: 0,
        step: 1,
        connect: "lower",
        range: {
    ...
    $('.sell .slider').on('change', function () {
        highlightLabel($(this).val());
    });

    $('.sell .slider').on('slide', function () {
        highlightLabel($(this).val());
    });

    $(".sell .slider-labels").on("click", "li", function () {
        $('.sell .slider').val($(this).index());
        highlightLabel($(this).index());
    });

    function highlightLabel($this) {
        $('.sell .slider-labels li').removeClass('active');

        var index = parseInt($this) + 1;
        for (let i = index; i > 0; i--) {
            var selector = '.sell .slider-labels li:nth-child(' + i + ')';
            $(selector).addClass('active');
        }
        $('.sell .slider-labels li').eq($this).click();

    }

});
$(function () {
    $(".Buy .slider").noUiSlider({
        start: 0,
        step: 1,
        connect: "lower",
        range: {
    ...
    $('.Buy .slider').on('change', function () {
        highlightLabel($(this).val());
    });

    $('.Buy .slider').on('slide', function () {
        highlightLabel($(this).val());
    });

    $(".Buy .slider-labels").on("click", "li", function () {
        $('.Buy .slider').val($(this).index());
        highlightLabel($(this).index());
    });

    function highlightLabel($this) {
        $('.Buy .slider-labels li').removeClass('active');

        var index = parseInt($this) + 1;
        for (let i = index; i > 0; i--) {
            var selector = '.Buy .slider-labels li:nth-child(' + i + ')';
            $(selector).addClass('active');
        }

        $('.Buy .slider-labels li').eq($this).click();
...
(function (c) {
    ...
    k.prototype.p = function (a, h, d, k) {
        ...
        this.update = !k;
        if ("string" ===
            typeof a && 0 === a.indexOf("-tooltip-")) this.K(a, h); else if ("string" === typeof a && 0 !== a.indexOf("-")) this.H(a); else if ("function" === typeof a) this.target = !1, this.method = a; else {
    ...
    function V(d, b, m) {
        ...
        function w(a, c, d, e) {
            ...
            c.on(a, function (a) {
                ...
                a.preventDefault();
                var c = 0 === a.type.indexOf("touch"), p = 0 === a.type.indexOf("mouse"),
                    F = 0 === a.type.indexOf("pointer"), g, k, l = a;
